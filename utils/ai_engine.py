...
_groq_index = 0
_gemini_index = 0
# Each credential gets its own circuit. A 429 on one Gemini project/key must
# not unnecessarily block the other configured projects. Groq keys in the same
# organization still share the provider's organization-level limits.
_groq_circuits: dict[str, _Circuit] = {}
_gemini_circuits: dict[str, _Circuit] = {}

def _circuit_for(pool: dict[str, _Circuit], key: str) -> _Circuit:
    circuit = pool.get(key)
    if circuit is None:
        circuit = _Circuit()
        pool[key] = circuit
    return circuit
...
async def _call_groq(system_prompt: str, user_prompt: str, schema: dict) -> tuple[Any, str]:
    """Try configured Groq credentials with per-key cooldowns.

    Important: multiple Groq API keys from the same organization do NOT create
    additional organization quota. Rotation here is failover/credential
    isolation, not a rate-limit bypass.
    """
    global _groq_index
    keys = _keys("GROQ_API_KEY", 10)
    if not keys:
    ...
    for offset in range(total):
        idx = (_groq_index + offset) % total
        key = keys[idx]
        circuit = _circuit_for(_groq_circuits, key)
        if not circuit.available():
            remaining = max(0, circuit.cooldown_until - time.monotonic())
            errors.append(f"key{idx+1} cooldown {remaining:.0f}s")
            continue
        ...
        try:
            ...
            circuit.success()
            _groq_index = (idx + 1) % total
            return data, f"Groq#{idx+1}"
        except Exception as exc:
            msg = str(exc)
            if _is_rate_limit(exc):
                delay = circuit.trip(_retry_after(exc), base=30, cap=300)
                errors.append(f"key{idx+1} rate-limit {delay:.0f}s")
                continue
            if _is_network_error(exc):
                delay = circuit.trip(base=15, cap=120)
                errors.append(f"key{idx+1} network {delay:.0f}s")
                continue
            errors.append(f"key{idx+1} {_status_code(exc) or ''}: {msg[:180]}")
            # Non-rate-limit errors are usually request/model/schema errors;
            # don't blindly hammer the same request with every credential.
            raise AIProviderError("Groq", "; ".join(errors), retryable=False) from exc
        finally:
            await _close_client(client)

    raise AIProviderError("Groq", "; ".join(errors), retryable=True, rate_limited=True)


async def _call_gemini(system_prompt: str, user_prompt: str, schema: dict) -> tuple[Any, str]:
    """Try up to 10 configured Gemini credentials/projects with per-key circuits."""
    global _gemini_index
    keys = _keys("GEMINI_API_KEY", 10)
    if not keys:
    ...
    total = len(keys)
    for offset in range(total):
        idx = (_gemini_index + offset) % total
        key = keys[idx]
        circuit = _circuit_for(_gemini_circuits, key)
        if not circuit.available():
            remaining = max(0, circuit.cooldown_until - time.monotonic())
            errors.append(f"key{idx+1} cooldown {remaining:.0f}s")
            continue
        ...
        try:
            ...
            circuit.success()
            _gemini_index = (idx + 1) % total
            return data, f"Gemini#{idx+1}"
        except Exception as exc:
            msg = str(exc)
            if _is_rate_limit(exc):
                delay = circuit.trip(_retry_after(exc), base=30, cap=300)
            ...
            if _is_network_error(exc):
                ...
                errors.append(f"key{idx+1} network {delay:.0f}s")
                continue
            # 5xx messages are sometimes not exposed as status_code by the SDK.
            # Treat explicit 503/unavailable/high-demand responses as transient.
            low = msg.lower()
            if _status_code(exc) in (500, 502, 503, 504) or any(x in low for x in ("503 unavailable", "service unavailable", "high demand")):
                delay = circuit.trip(base=5, cap=60)
                errors.append(f"key{idx+1} temporary-503 {delay:.0f}s")
                continue
            errors.append(f"key{idx+1} {_status_code(exc) or ''}: {msg[:180]}")
    ...
    raise AIProviderError("Gemini", "; ".join(errors), retryable=True, rate_limited=True)


async def solve_text_batch(system_prompt: str, user_prompt: str) -> tuple[list[dict], str]:
    """Groq primary -> Gemini fallback; each provider may fail over across 10 configured credentials."""
    errors = []
