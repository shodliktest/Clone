...
from utils.ram_cache import get_test_by_id, is_test_paused, get_test_meta, protect_content_for_chat
...
async def _send_question_new(bot, cid, state, uid):
    ...
    if photo_id_first:
        try:
            await bot.send_photo(cid, photo_id_first, protect_content=protect_content_for_chat(cid))
        except Exception as e:
            log.error(f"Inline rasm xato: {e}")
    text, kb, is_text = _build_question_content(qs, idx, time_left=QUESTION_SEC)
    msg = await bot.send_message(cid, text, reply_markup=kb,
    ...
    if qs and ans:
        ...
        try:
            await callback.bot.send_message(uid, "🏠 Asosiy menyu:",
                reply_markup=main_kb(uid), protect_content=protect_content_for_chat(cid))
        except Exception:
            pass
